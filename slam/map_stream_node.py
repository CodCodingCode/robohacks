...
Usage:
    python3 /home/jetson1/robohacks/slam/map_stream_node.py \
...
class MapStreamNode(Node):
    ...
    def snapshot(self) -> tuple[tuple[float, float, float] | None, OccupancyGrid | None]:
        with self._lock:
            return self._pose, self._last_map


def build_robot_payload(pose: tuple[float, float, float]) -> dict:
    x, y, theta = pose
    return {"x": x, "y": y, "theta": theta, "battery": 100}
...
async def serve(node: MapStreamNode, host: str, port: int) -> None:
    ...
    async def broadcaster() -> None:
        ...
        last_map_push = 0.0
        while True:
            pose, occ = node.snapshot()
            payload: dict = {
                "timestamp": time.time(),
                "mission_phase": "recon",
            }
            if pose is not None:
                payload["robot"] = build_robot_payload(pose)
            now = time.time()
