...
(function (global) {
  ...
  function initRadar() {
    ...
    if (scopeCanvas) {
      ...
      // Run the scope animation independently at ~30fps
      _animLoop();
    }
  }
  ...
  function _drawScope(targets) {
    ...
    const cx = w / 2;
    const cy = h - 10;
    const scale = (h - 20) / SCOPE_RANGE_M;
    const now = Date.now();

    // Sweep angle: rotates from -PI to 0 (left to right across the top half)
    ...
    // Range rings
    scopeCtx.strokeStyle = "rgba(40,75,201,0.12)";
    ...
    // Range labels
    scopeCtx.fillStyle = "rgba(255,255,255,0.12)";
    scopeCtx.font = "8px 'JetBrains Mono', monospace";
    scopeCtx.textAlign = "center";
    for (let r = 2; r <= SCOPE_RANGE_M; r += 2) {
      scopeCtx.fillText(r + "m", cx + 14, cy - r * scale + 4);
    }

    // FOV wedge fill
    scopeCtx.fillStyle = "rgba(40,75,201,0.04)";
    scopeCtx.beginPath();
    ...
    const trailSpan = 0.4; // radians of trail
    const sweepGrad = scopeCtx.createConicGradient(
      sweepAngle - trailSpan + Math.PI / 2,
      0,
      0,
    );
    ...
    // --- Afterglow blips (fading old detections) ---
    for (const blip of blipHistory) {
      ...
      scopeCtx.beginPath();
      scopeCtx.arc(bx, by, 2, 0, Math.PI * 2);
      scopeCtx.fill();
      scopeCtx.restore();
    }
    ...
    for (const t of targets) {
      ...
      // Pulse glow
      const pulse = 6 + Math.sin(now / 300 + (t.id || 0)) * 2;
      scopeCtx.save();
      ...
      scopeCtx.fillStyle = "#284bc9";
      scopeCtx.beginPath();
      scopeCtx.arc(sx, sy, 3, 0, Math.PI * 2);
      scopeCtx.fill();

      // Bright center
      scopeCtx.fillStyle = "rgba(120,160,255,0.8)";
      scopeCtx.beginPath();
      scopeCtx.arc(sx, sy, 1.2, 0, Math.PI * 2);
      scopeCtx.fill();
      scopeCtx.restore();
    }
    ...
    scopeCtx.fillStyle = "#284bc9";
    scopeCtx.beginPath();
    scopeCtx.moveTo(cx, cy - 5);
    scopeCtx.lineTo(cx - 4, cy + 3);
    scopeCtx.lineTo(cx + 4, cy + 3);
    scopeCtx.closePath();
    scopeCtx.fill();

    // Status overlay
    scopeCtx.font = "500 9px 'JetBrains Mono', monospace";
    scopeCtx.textAlign = "left";
    if (targets.length > 0) {
      scopeCtx.fillStyle = "#284bc9";
      scopeCtx.fillText(targets.length + " TARGET" + (targets.length > 1 ? "S" : ""), 4, 12);
    } else {
      // Scanning indicator with animated dots
      const dots = ".".repeat(1 + (Math.floor(now / 500) % 3));
      scopeCtx.fillStyle = "rgba(40,75,201,0.5)";
      scopeCtx.fillText("SCANNING" + dots, 4, 12);
    }
  }
